...
export class NutritionOptimizer {
  ...
  async generateNutritionInsights(
    ...
    const budgetThreshold = profile.budgetConstraint === 'low' ? 15 : 
                         typeof profile.budgetConstraint === 'number' ? profile.budgetConstraint : 25;
    if (profile.budgetConstraint && cost > budgetThreshold) { // Daily cost threshold
      insights.push({
        type: 'efficiency',
        severity: 'low',
