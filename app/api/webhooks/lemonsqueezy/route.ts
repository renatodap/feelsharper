...
export async function POST(request: NextRequest) {
  try {
    ...
    const isValid = process.env.LEMONSQUEEZY_WEBHOOK_SECRET 
      ? verifyWebhookSignature(rawBody, signature)
      : true; // Allow in development without secret
    ...
    if (!process.env.LEMONSQUEEZY_WEBHOOK_SECRET) {
      console.warn('WARNING: Webhook signature verification disabled. Configure LEMONSQUEEZY_WEBHOOK_SECRET for production!');
    }
